...
class Pacientes extends MY_Controller {
	...
	function index(){
		...
		$countPacientes = $this->db
							...
							->from("pacientes AS p")
							->join("consultas AS c", "p.id = c.pacientes_id")
							->join("planos AS pl", "pl.id = c.planos_id")
							->get()->row();
		...
		$resultPacientes = $this->db
									->select("p.*, pl.nome_plano, c.status AS status_consulta")
									->from("pacientes AS p")
									->join("consultas AS c", "p.id = c.pacientes_id")
									->join("planos AS pl", "pl.id = c.planos_id")
									->order_by("p.nome_pac ASC, c.status DESC")
